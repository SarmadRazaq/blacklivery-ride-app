...
import { ADMIN_PROMOTIONS, adminPromotionById } from '../api/endpoints';
import { REGIONS, DEFAULT_REGION_CODE, getCurrencySymbol, getRegion } from '../config/regions';
...
const EditPromotionForm = ({ promotion, onCancel, onSuccess }: { promotion: Promotion; onCancel: () => void; onSuccess: () => void }) => {
    const { register, handleSubmit, formState: { errors } } = useForm<PromotionFormValues>({
        defaultValues: {
            code: promotion.code,
            description: promotion.description,
            discountType: promotion.discountType,
            amount: String(promotion.amount),
            ...
            regions: promotion.regions && promotion.regions.length > 0 ? promotion.regions[0] : DEFAULT_REGION_CODE,
            active: promotion.active ? 'true' : 'false'
        }
    });
    ...
    const onSubmit = async (data: PromotionFormValues) => {
        try {
            const payload = {
                ...
                maxRedemptions: parseInt(data.maxRedemptions, 10),
            ...
            await api.put(adminPromotionById(promotion.id), payload);
            toast.success('Promotion updated successfully');
            onSuccess();
        } catch (error: unknown) {
            const apiError = error as ApiError;
            console.error('Update promotion error:', apiError.response?.data);
            toast.error(apiError.response?.data?.message || 'Failed to update promotion');
        }
    };
...
const PromotionsPage = () => {
    ...
    const fetchPromotions = async () => {
        setLoading(true);
        try {
            const response = await api.get(ADMIN_PROMOTIONS);
    ...
    const onSubmit = async (data: PromotionFormValues) => {
        try {
            const payload = {
                ...
                amount: parseFloat(data.amount),
                maxRedemptions: parseInt(data.maxRedemptions, 10),
                regions: data.regions ? [data.regions] : [DEFAULT_REGION_CODE], // Simplified region select
                startsAt: new Date(data.startsAt).toISOString(),
                endsAt: new Date(data.endsAt).toISOString()
            };

            await api.post(ADMIN_PROMOTIONS, payload);
                                ...
                                promotions.map((promo) => (
                                    <React.Fragment key={promo.id}>
                                        <TableRow className={expandedPromoId === promo.id ? 'bg-blue-50' : ''}>
                                            ...
                                            <TableCell>
                                                {promo.discountType === 'percentage' ? `${promo.amount}%` : `${getCurrencySymbol(getRegion(promo.regions?.[0] || DEFAULT_REGION_CODE).currency)}${promo.amount}`}
                                            </TableCell>
