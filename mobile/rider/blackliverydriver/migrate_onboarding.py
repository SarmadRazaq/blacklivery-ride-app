...
files = glob.glob('lib/features/onboarding/*.dart')
...
for file in files:
    ...
    content = content.replace('context.read<AuthProvider>()', 'ref.read(authRiverpodProvider)')
    content = content.replace('context.watch<AuthProvider>()', 'ref.watch(authRiverpodProvider)')
    content = content.replace('context.read<RegionProvider>()', 'ref.read(regionRiverpodProvider)')
    content = content.replace('context.read<RideProvider>()', 'ref.read(rideRiverpodProvider)')
    content = content.replace('context.read<DriverProvider>()', 'ref.read(driverRiverpodProvider)')
    
    # Replace Consumer
    content = re.sub(r'Consumer<AuthProvider>\(\s*builder: \(context, (\w+), child\) {', r'Consumer(\n  builder: (context, ref, child) {\n    final \1 = ref.watch(authRiverpodProvider);', content)
