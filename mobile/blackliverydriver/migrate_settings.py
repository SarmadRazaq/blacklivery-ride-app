...
files = glob.glob('lib/features/settings/*.dart')

for file in files:
    with open(file, 'r', encoding='utf-8') as f:
        content = f.read()
    
    if 'context.read' not in content and 'context.watch' not in content:
        continue
    ...
    content = content.replace('context.read<provider.AuthProvider>()', 'ref.read(authRiverpodProvider)')
